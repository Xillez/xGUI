...
local widgetBase = {
	...
	position = nil,             -- Widget position
    dimention = nil,            -- Widget dimention
    center = nil                -- Widget center position
}

oop.make(widgetBase)
function widgetBase:construct(parent)       -- Constructor (executed in "oop" library)
	self.parent = parent        -- Parent widget
	self.children = {}          -- Children widgets, no children yet
    self.position = vector(0, 0),    -- Widget position
    self.dimention = vector(0, 0),   -- Widget dimention
    self.center = vector(0, 0)       -- Widget center position
